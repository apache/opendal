...
The leading prototype candidate uses an exact-build shared native runtime.
...
- [Compatibility and ABI](compatibility.md) defines version identities,
...
- A service or layer package can be installed without rebuilding the base
  language binding.
- The base binding loads without dependencies required by unselected packages.
  In particular, HDFS must not make Java, Hadoop, `libhdfs`, or `libjvm` a base
  requirement.
- A service package owns its configuration schema, URI interpretation,
  credentials, and redaction behavior.
...
descriptor, rules, and conformance suite is required; sharing one physical
runtime library across Python, Ruby, and Node.js installations is not.
...
language API across all three bindings. The manifest, bootstrap descriptor,
...
- A text manifest with the package ID, provided schemes or layers, native
  artifact path, unique entry symbol, and required runtime build ID.
- A target-specific native library, or source needed to build one.
- Package-specific documentation and conformance tests.

Registering the text manifest must not load the native library. The runtime
...
A native extension contains one or more service or layer factories. The
package exports one generated, package-unique bootstrap symbol and hides other
package-local symbols where the platform permits it.

The bootstrap uses a small C-layout descriptor so the runtime can inspect its
size, revision, manifest identity, target, and runtime build ID safely. Only
after all checks pass may the runtime enter the exact-build internal interface.
That internal interface is not a stable Rust ABI.
...
same package identity in the returned descriptor.
...
ConfigRequest {
    package_schema_revision,
    structured_values,
}
```

`structured_values` uses the versioned, language-neutral `ConfigValue` grammar
...
- One manifest can claim several names and aliases atomically.
- Re-registering the identical package and manifest is idempotent.
- A different owner claiming an existing name rejects the entire manifest.
...
Each ecosystem needs a runtime distribution, independent extension packages,
and a compatibility aggregator:

```text
runtime distribution       minimal binding and native runtime
service/layer packages     exact dependency on that runtime release
compatibility aggregator   runtime plus the services/layers shipped today
```

The aggregator preserves existing installation commands during migration. A
minimal installation uses the runtime distribution plus selected extension
packages. Independently installable does not imply independently
ABI-versioned: every exact-runtime native package must be rebuilt for a new
runtime build ID.
...
The host still owns these in-scope properties:
