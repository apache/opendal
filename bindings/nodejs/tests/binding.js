...
Then('The blocking file {string} must have content {string}', (path, content) => {
...
When('Async write path {string} with content {string}', async (path, content) => {
    await this.op.write(path, content)
});

Then('The async file {string} should exist', async (path) => {
    await this.op.stat(path)
});

Then('The async file {string} entry mode must be file', async (path) => {
    let meta = await this.op.stat(path);
    assert(meta.isFile())
});

Then('The async file {string} content length must be "{int}"', async (path, size) => {
    let meta = await this.op.stat(path);
    assert(meta.contentLength == size)
});

Then('The async file {string} must have content {string}', async (path, content) => {
