...
let test_operator_writer test_ctxt =
  ...
  ignore (test_check_result (Operator.Writer.write writer (Bytes.of_string "Hello ")));
  ignore (test_check_result (Operator.Writer.write writer (Bytes.of_string "World!")));
...
let test_is_exist test_ctxt =
  ...
  ignore (test_check_result (Operator.write bo "testfile.txt" (Bytes.of_string "test")));
...
let test_copy test_ctxt =
  ...
  ignore (test_check_result (Operator.write bo "source.txt" (Bytes.of_string content)));
  ...
  let source_str = source_data |> Array.to_seq |> Bytes.of_seq |> Bytes.to_string in
...
let test_rename test_ctxt =
  ...
  ignore (test_check_result (Operator.write bo "old_name.txt" (Bytes.of_string content)));
...
let test_delete test_ctxt =
  ...
  (* Create a file *)
  ignore (test_check_result (Operator.write bo "to_delete.txt" (Bytes.of_string "delete me")));
  
  (* Verify it exists *)
  let exists_before = Operator.is_exist bo "to_delete.txt" |> test_check_result in
  ...
  (* Verify it's gone *)
  let exists_after = Operator.is_exist bo "to_delete.txt" |> test_check_result in
...
let test_remove test_ctxt =
  ...
  Array.iter (fun file ->
    ignore (test_check_result (Operator.write bo file (Bytes.of_string ("content of " ^ file))))
  ) files;
...
let test_complex_workflow test_ctxt =
  ...
  let readme_writer = Operator.writer bo "project/README.md" |> test_check_result in
  ignore (test_check_result (Operator.Writer.write readme_writer (Bytes.of_string "# My Project\n")));
  ignore (test_check_result (Operator.Writer.write readme_writer (Bytes.of_string "This is a test project.\n")));
  ignore (test_check_result (Operator.Writer.close readme_writer));
  
  (* Create a source file *)
  ignore (test_check_result (Operator.write bo "project/src/main.ml" (Bytes.of_string "let () = print_endline \"Hello, World!\"")));
  
  (* List the project directory *)
  let entries = Operator.list bo "project/" |> test_check_result in
  assert_equal 4 (Array.length entries); (* project/, src/, docs/, README.md *)
