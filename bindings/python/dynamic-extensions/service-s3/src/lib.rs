...
static API: ExtensionApiV1 = ExtensionApiV1 {
    ...
    package_id: ByteSlice::from_static("opendal-service-s3-poc"),
    component_id: ByteSlice::from_static("s3"),
    entry_symbol: ByteSlice::from_static("opendal_service_s3_bootstrap_v1"),
    create_operator,
    destroy_operator,
    operator_info,
    operator_write,
...
static TOKIO_RUNTIME: std::sync::LazyLock<tokio::runtime::Runtime> =
    std::sync::LazyLock::new(|| {
        tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .build()
            .expect("prototype S3 runtime should build")
    });
...
unsafe extern "C" fn create_operator(
    ...
    catch_unwind(AssertUnwindSafe(|| unsafe {
        ...
        *operator = std::ptr::null_mut();
        ...
        for (key, value) in options {
            builder = match key {
                "bucket" => builder.bucket(value),
                "region" => builder.region(value),
                "root" => builder.root(value),
                "endpoint" => builder.endpoint(value),
                "access_key_id" => builder.access_key_id(value),
                "secret_access_key" => builder.secret_access_key(value),
                _ => return fail(error, format!("unsupported S3 option {key}")),
            };
        }
    ...
    .unwrap_or_else(|_| fail(error, "S3 operator construction panicked"))
}
...
unsafe fn as_operator<'a>(operator: *mut c_void) -> Result<&'a Operator, &'static str> {
    if operator.is_null() {
        return Err("S3 operator is null");
    }
...
unsafe extern "C" fn operator_info(
    ...
    .unwrap_or_else(|_| fail(error, "S3 operator info panicked"))
}
...
unsafe extern "C" fn operator_write(
    ...
    .unwrap_or_else(|_| fail(error, "S3 write panicked"))
}
...
unsafe extern "C" fn operator_read(
    ...
    .unwrap_or_else(|_| fail(error, "S3 read panicked"))
}

#[unsafe(no_mangle)]
/// Returns the S3 extension function table.
///
/// # Safety
///
/// `extension` must point to writable storage for an extension table pointer.
pub unsafe extern "C" fn opendal_service_s3_bootstrap_v1(
