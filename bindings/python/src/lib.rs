...
// Python wrapper for opendal::raw::Timestamp that can be converted to/from Python datetime
#[repr(transparent)]
#[derive(Clone, Debug)]
pub struct Timestamp(ocore::raw::Timestamp);

impl From<ocore::raw::Timestamp> for Timestamp {
    fn from(ts: ocore::raw::Timestamp) -> Self {
        Self(ts)
    }
}

impl From<Timestamp> for ocore::raw::Timestamp {
    fn from(ts: Timestamp) -> Self {
        ts.0
    }
}

// Convert Timestamp to Python datetime using jiff-02 feature
impl<'py> pyo3::conversion::IntoPyObject<'py> for Timestamp {
    type Target = PyAny;
    type Output = Bound<'py, Self::Target>;
    type Error = PyErr;

    fn into_pyobject(self, py: Python<'py>) -> Result<Self::Output, Self::Error> {
        // Convert inner Timestamp to jiff::Timestamp,
        // which PyO3's jiff-02 feature will convert to Python datetime
        let py_datetime = self.0.into_inner().into_pyobject(py)?;
        Ok(py_datetime.into_any())
    }
}

// Convert Python datetime to Timestamp using jiff-02 feature
impl<'py> FromPyObject<'py> for Timestamp {
    fn extract_bound(ob: &Bound<'py, PyAny>) -> PyResult<Self> {
        // Extract jiff::Timestamp from Python datetime using jiff-02 feature
        let jiff_ts: jiff::Timestamp = ob.extract()?;
        Ok(Timestamp(ocore::raw::Timestamp::from(jiff_ts)))
    }
}
