...
pub(crate) struct RenameArgs {
    pub(crate) opts: options::RenameOptions,
    pub(crate) to: String,
}
...
///
/// Users can add more options by public functions provided by this struct.
pub struct FutureRename<F: Future<Output = Result<()>>> {
    inner: OperatorFuture<RenameArgs, (), F>,
}

impl<F: Future<Output = Result<()>>> FutureRename<F> {
    pub(crate) fn new(inner: OperatorFuture<RenameArgs, (), F>) -> Self {
        Self { inner }
    }
    ...
    /// ```
    pub fn if_not_exists(mut self, v: bool) -> Self {
        self.inner.args.opts.if_not_exists = v;
        self
    }
}

impl<F> IntoFuture for FutureRename<F>
where
