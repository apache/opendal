...
/// Outlasts the burst of ranges one scattered read fans over a file.
...
pub struct HfConfig {
    ...
    /// `/resolve` again. Default `10s`; `0s` disables the cache.
    ///
    /// A write or delete through this operator discards the cache outright,
    /// so it never hides your own changes. This bounds how long a change made
    /// *elsewhere* to a floating revision such as `main` stays invisible.
