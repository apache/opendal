...
mod tests {
    ...
    struct MockHttpClient {
        ...
        responses: Arc<StdMutex<VecDeque<(StatusCode, &'static str)>>>,
    }
    ...
    impl HttpFetch for MockHttpClient {
        async fn fetch(&self, req: Request<Buffer>) -> Result<Response<HttpBody>> {
            self.requests.lock().expect("lock poisoned").push(format!(
                "{} {}",
                req.method(),
                req.uri()
            ));
            ...
            let (status, body) = self
                ...
                .pop_front()
                .expect("response queue must not be empty");
            let bs = Bytes::from(body.to_string());
            let size = bs.len() as u64;
            let body = HttpBody::new(stream::iter(vec![Ok(Buffer::from(bs))]), Some(size));

            Ok(Response::builder().status(status).body(body).unwrap())
        }
    }

    fn folder_item_json(name: &str, parent_path: &str, id: &str) -> String {
        format!(
            r#"{{
                "id": "{id}",
                "name": "{name}",
    ...
    fn create_test_core(mock: MockHttpClient) -> OneDriveCore {
        let info = AccessorInfo::default();
        info.set_scheme(ONEDRIVE_SCHEME).set_root("/");
        info.update_http_client(|_| HttpClient::with(mock));

        let info = Arc::new(info);
        let mut signer = OneDriveSigner::new(info.clone());
        signer.access_token = "access_token".to_string();
    ...
    #[tokio::test]
    async fn test_ensure_directory_creates_missing_ancestors() {
        let root = Box::leak(folder_item_json("root", "/drive/root:", "root").into_boxed_str());
        let a = Box::leak(folder_item_json("a", "/drive/root:", "a").into_boxed_str());
        let ab = Box::leak(folder_item_json("b", "/drive/root:/a", "ab").into_boxed_str());
        let abc = Box::leak(folder_item_json("c", "/drive/root:/a/b", "abc").into_boxed_str());
        ...
        let mock = MockHttpClient::new(vec![
            ...
            (StatusCode::NOT_FOUND, r#"{"error":"not found"}"#),
            (StatusCode::OK, root),
            (StatusCode::CREATED, a),
            (StatusCode::CREATED, ab),
            (StatusCode::CREATED, abc),
        ]);
        let core = create_test_core(mock.clone());

        let item = core
            .ensure_directory("a/b/c/")
            .await
            .expect("ensure directory must succeed");
