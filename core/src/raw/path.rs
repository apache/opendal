...
pub fn build_tmp_path_of(path: &str) -> String {
    ...
    //
    // 1. `fastrand` is roughly 10× faster than `SipHasher13`, but it adds an extra dependency.
    ...
    let state = std::collections::hash_map::RandomState::new().build_hasher();
    for _ in 0..RANDOM_TMP_PATH_POSTFIX_LENGTH {
        let choice: usize = (state.finish() % CHARS_LENGTH).try_into().unwrap();
        buf.push(CHARS[choice] as char);
        // fastrand uses a fast Wyrand pseudo-number generator. If the randomness is not enough,
        // we can:
        // 1. set a longer length
        // 2. add `getrandom` to initialize fastrand's RPNG
    }
