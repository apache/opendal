...
pub fn tests(op: &Operator, tests: &mut Vec<Trial>) {
    ...
    if cap.read && cap.write && cap.rename && cap.rename_with_if_not_exists {
        tests.extend(async_trials!(
            op,
            test_rename_with_if_not_exists_to_new_file,
            test_rename_with_if_not_exists_to_existing_file
        ))
    }
}
...
/// Rename to a nonexistent path when `if_not_exists` is enabled.
pub async fn test_rename_with_if_not_exists_to_new_file(op: Operator) -> Result<()> {
    let source_path = uuid::Uuid::new_v4().to_string();
    let (source_content, _) = gen_bytes(op.info().capability());
...
/// Rename to an existing path should fail when `if_not_exists` is enabled.
pub async fn test_rename_with_if_not_exists_to_existing_file(op: Operator) -> Result<()> {
    let source_path = uuid::Uuid::new_v4().to_string();
    let (source_content, _) = gen_bytes(op.info().capability());
    ...
    let actual_source_content = op
        .read(&source_path)
        .await
        .expect("read must succeed")
        .to_bytes();
    assert_eq!(
        sha256_digest(actual_source_content),
        sha256_digest(&source_content),
    );

    let actual_target_content = op
        .read(&target_path)
        .await
        .expect("read must succeed")
        .to_bytes();
    assert_eq!(
        sha256_digest(actual_target_content),
        sha256_digest(&target_content),
    );
