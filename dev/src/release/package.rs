...
pub fn all_packages() -> Vec<Package> {
    let core = make_package("core", "0.53.2", vec![]);

    // Integrations
    let cloud_filter = make_package("integrations/cloud_filter", "0.0.9", vec![core.clone()]);
    ...
    // Binaries
    let oay = make_package("bin/oay", "0.41.20", vec![core.clone(), dav_server.clone()]);
    let ofs = make_package(
        "bin/ofs",
        "0.0.21",
        vec![core.clone(), fuse3.clone(), cloud_filter.clone()],
    );
    let oli = make_package("bin/oli", "0.41.20", vec![core.clone()]);

    // Bindings
    let c = make_package("bindings/c", "0.46.0", vec![core.clone()]);
    let cpp = make_package("bindings/cpp", "0.45.20", vec![core.clone()]);
    let java = make_package("bindings/java", "0.47.12", vec![core.clone()]);
    let nodejs = make_package("bindings/nodejs", "0.48.2", vec![core.clone()]);
    let python = make_package("bindings/python", "0.45.19", vec![core.clone()]);
