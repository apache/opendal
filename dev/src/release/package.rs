...
pub fn all_packages() -> Vec<Package> {
    let core = make_package("core", "0.53.1", vec![]);

    // Integrations
    let cloud_filter = make_package("integrations/cloud_filter", "0.0.8", vec![core.clone()]);
    let dav_server = make_package("integrations/dav-server", "0.5.1", vec![core.clone()]);
    let fuse3 = make_package("integrations/fuse3", "0.0.15", vec![core.clone()]);
    let object_store = make_package("integrations/object_store", "0.51.1", vec![core.clone()]);
    let parquet = make_package("integrations/parquet", "0.5.1", vec![core.clone()]);
    let unftp_sbe = make_package("integrations/unftp-sbe", "0.1.1", vec![core.clone()]);

    // Binaries
    let oay = make_package("bin/oay", "0.41.19", vec![core.clone(), dav_server.clone()]);
    let ofs = make_package(
        "bin/ofs",
        "0.0.20",
        vec![core.clone(), fuse3.clone(), cloud_filter.clone()],
    );
    let oli = make_package("bin/oli", "0.41.19", vec![core.clone()]);

    // Bindings
    let c = make_package("bindings/c", "0.45.7", vec![core.clone()]);
    let cpp = make_package("bindings/cpp", "0.45.19", vec![core.clone()]);
    let java = make_package("bindings/java", "0.47.11", vec![core.clone()]);
    let nodejs = make_package("bindings/nodejs", "0.48.1", vec![core.clone()]);
    let python = make_package("bindings/python", "0.45.18", vec![core.clone()]);
