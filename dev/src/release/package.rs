...
pub fn all_packages() -> Vec<Package> {
    ...
    let unftp_sbe = make_package("integrations/unftp-sbe", "0.4.8", vec![core.clone()]);

    // Binaries moved to separate repositories; no longer released from this repo

    // Bindings
    let c = make_package("bindings/c", "0.47.5", vec![core.clone()]);
    let cpp = make_package("bindings/cpp", "0.45.32", vec![core.clone()]);
    let java = make_package("bindings/java", "0.50.5", vec![core.clone()]);
    let nodejs = make_package("bindings/nodejs", "0.49.10", vec![core.clone()]);
    let python = make_package("bindings/python", "0.47.9", vec![core.clone()]);
    let ruby = make_package("bindings/ruby", "0.1.13", vec![core.clone()]);
    let dotnet = make_package("bindings/dotnet", "0.2.2", vec![core.clone()]);
