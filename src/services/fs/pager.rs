    ...
    async fn next(&mut self) -> Result<Option<Vec<oio::Entry>>> {
        ...
        for _ in 0..self.size {
            ...
            let d = if file_type.is_file() {
                oio::Entry::new(&rel_path, Metadata::new(ObjectMode::FILE))
            } else if file_type.is_dir() {
                // Make sure we are returning the correct path.
                oio::Entry::new(&format!("{rel_path}/"), Metadata::new(ObjectMode::DIR))
            } else {
                oio::Entry::new(&rel_path, Metadata::new(ObjectMode::Unknown))
            };
